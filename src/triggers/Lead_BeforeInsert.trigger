/**
    About
    -----
    Description: Before Insert Trigger on Lead
...
*/
trigger Lead_BeforeInsert on Lead (before insert) {
    ...
    if(!UTIL_OrgSettings.isTriggersDisabled()) {	// Allow users to disable Triggers.
	    ...
	    for(Lead lead : trigger.new) {
	        ...
	        if(lead.Account__c != null && lead.Status != null && lead.Sales_Area__c != null) {
	            if(leadStatus.contains(lead.Status) && leadSalesArea.contains(lead.Sales_Area__c)) {
